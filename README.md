...
- 左栏按项目分组列全部会话，最近的在上
- **搜索**：即时按标题模糊子序列过滤；停顿 ~0.3s 跨全部会话搜正文，带命中数与片段高亮
- **筛选**：项目下拉 + 时间范围（今天 / 7 天 / 30 天）+「含思考」开关（把 thinking 纳入搜索）
- **Markdown 渲染**：正文与思考按 Markdown 渲染（标题/粗斜体/行内码/代码块/列表/引用/链接），先转义再渲染防 XSS；搜索高亮用哨兵字符穿透到渲染后的 HTML
- **对话渲染**：气泡式；思考 / 工具调用 / 工具结果默认折叠可展开
- **导出 MD**：顶栏一键把当前会话导出 Markdown
